...
export function validateProposal(input: unknown, expectedFleet?: number): ProposalValidation {
  ...
  if (!p || typeof p !== "object")
    return { valid: false, errors: ["The proposal is not an object."] };
  ...
  if (!CHOREOGRAPHY_CONCEPTS.includes(p.concept))
    errors.push(`Unsupported concept "${String(p.concept)}".`);
  if (!Number.isInteger(p.fleetCount) || p.fleetCount <= 0)
    errors.push("The fleet count must be a positive integer.");
  ...
  if (
    expectedFleet !== undefined &&
    Number.isInteger(p.fleetCount) &&
    p.fleetCount > expectedFleet
  ) {
    errors.push(
      `The proposal targets ${p.fleetCount} drones but the project has only ${expectedFleet}.`,
    );
  }

  const spec = p.formationSpec;
  if (!spec || !num(spec.width) || spec.width <= 0)
    errors.push("formationSpec.width must be positive.");
  ...
  if (!anim || typeof anim.dynamic !== "boolean") errors.push("animationSpec.dynamic is missing.");
    ...
    if (!num(anim.cycleDuration) || anim.cycleDuration <= 0)
      errors.push("animationSpec.cycleDuration must be positive.");
    if (!Number.isInteger(anim.cycles) || anim.cycles <= 0)
      errors.push("animationSpec.cycles must be a positive integer.");
    if (!num(anim.amplitudeDeg) || anim.amplitudeDeg < 0)
      errors.push("animationSpec.amplitudeDeg must be >= 0.");
    if (!["NONE", "REPEAT", "PING_PONG"].includes(anim.loop))
      errors.push("animationSpec.loop is invalid.");
  }
  ...
  if (!timing || !num(timing.hold) || timing.hold <= 0)
    errors.push("timing.hold must be positive.");
  ...
  if (!p.provenance || typeof p.provenance.providerId !== "string")
