...
/**
 * A glyph pack is a complete, versioned geometry source: either open strokes
 * (single-line studio pack) or closed contours flattened from a REAL typeface
 * at build time (see scripts/buildTypefacePack.mjs). Runtime never touches a
 * font file, so identical recipes stay byte-identical everywhere.
 */
export interface GlyphPackDefinition {
  readonly id: string;
  readonly version: number;
  /** Operator-facing name. */
  readonly label: string;
  /** Cap height in the pack's own em units. */
  readonly capHeight: number;
  /** True when each stroke is a closed contour (real typeface outline). */
  readonly closedContours: boolean;
  readonly glyphs: Readonly<Record<string, Glyph>>;
}

export const STROKE_PACK: GlyphPackDefinition = {
  id: GLYPH_PACK_ID,
  version: GLYPH_PACK_VERSION,
  label: "Studio strokes (single line)",
  capHeight: CAP_HEIGHT,
  closedContours: false,
  glyphs: GLYPH_PACK,
};
