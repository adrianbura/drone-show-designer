...
describe("dbg", () => {
  it("groups", () => {
    ...
    const a = makeSceneLocalFormation("f-a", "A", "line", 110, base.area, { length: 90, rows: 1 });
    ...
    const r1 = addObject(project, scene, { source: { kind: "STATIC", formationId: a.id }, name: "A", requestedDroneCount: 110, position: [0,0,0] });
    ...
    const p = plan.participation.find((x) => x.clipId === "c")!;
